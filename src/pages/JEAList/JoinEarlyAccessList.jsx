...
const JoinEarlyAccessList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Checkbox validation added here
    if (!formData.agree) {
    ...
    try {
      ...
      if (response.ok) {
        alert(data.message || "Successfully joined Early Access!");
  ...
  <div className="relative z-10 bg-gray/30 backdrop-blur-lg p-8 rounded-2xl shadow-2xl w-full max-w-md border border-white/20 text-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={loading || !formData.agree}  // ✅ button disabled until checked
...
export default JoinEarlyAccessList;
